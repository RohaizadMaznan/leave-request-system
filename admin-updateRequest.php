...
$sqlR = "SELECT user.*, applyleave.*, leavereason.* FROM applyleave JOIN user ON applyleave.apply_userId=user.userId JOIN leavereason ON applyleave.apply_leaveRequestId=leavereason.leaveId WHERE applyleave.applyId = '".$_REQUEST['id']."'";
  ...
  $sql =  "UPDATE applyleave SET  apply_leaveRequestId = '".$_REQUEST['reasonLeave']."', 
    ...
    <script>
      ...
      location.assign("admin-index.php?post_type=list-leave-applied&submitted=success");
...
?>
                                        ...
                                        <h4 class="header-title">Application - Leave Request by Staff</h4>
                                        <p>Please make sure admin make a clear and confirmed details on accepting or rejecting the request.</p>
                                        <hr/>
                                        <?php while($row4 = mysqli_fetch_assoc($outReq)) { ?>
                                        <form action="" method="post">

                                            <div class="form-group">
                                            <label for="reasonLeave">Declaration Application</label>
                                                <select class="form-control" id="reasonLeave" name="status" value="<?php echo $row4['leaveId']?>" style="height: 50px">
                                                  <option><?php echo $row4['apply_status']?> [Current]</option>
                                                  <option value="Accepted">1 - Accepted</option>
                                            ...
                                            <div class="form-group">
                                                <label for="totalDays">Total Days (Admin may reduce the total days leave)</label>
